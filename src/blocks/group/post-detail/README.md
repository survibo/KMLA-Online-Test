...
- 상세 화면 댓글도 drawer와 같은 답글 접기/펼치기 thread UI를 공유한다.
- 댓글 아이콘 숫자는 direct child count 기준이다.
- 대표 액션 버튼은 `shadcn/ui`의 `Button` 기반으로 유지한다.
- 상세 제목 타이포는 목록 카드와 과하게 벌어지지 않도록 같은 밀도 범위를 유지한다.
- 게시글 헤더의 `...` 메뉴까지 포함해, 헤더/본문/이미지/액션 영역은 `src/blocks/group/shared.tsx`의 공용 post summary를 카드와 같은 폭/패딩 기준으로 그대로 사용한다.
- 상세 화면의 게시글 `...` menu drawer는 상세 block이 단일 인스턴스로 관리하고, 필요하면 `menu=<postId>` query param으로 뒤로가기 흐름을 맞춘다.
- 상세 화면의 댓글 입력창은 모바일 키보드와 스크롤 영향이 덜하도록 화면 하단 `fixed` bar로 두고, 댓글 목록 마지막 줄이 가려지지 않도록 입력창 실측 높이만큼 본문 하단 여백을 함께 확보한다.
